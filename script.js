...
function decrypt(cipher, key) {
    ...
    let text = '';
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) text += grid[r][c];
    }
    return text;
}
...
document.getElementById('generateKeysBtn').addEventListener('click', () => {
    ...
    if (!p || !q || p === q) {
        alert('Enter two different prime numbers.');
        return;
    }

    rsaKeys = generateRSA(p, q);

    document.getElementById('publicKey').value = `(${rsaKeys.publicKey[0]}, ${rsaKeys.publicKey[1]})`;
    document.getElementById('privateKey').value = `(${rsaKeys.privateKey[0]}, ${rsaKeys.privateKey[1]})`;
});
...
document.getElementById('rsaEncryptBtn').addEventListener('click', () => {
    ...
    document.getElementById('rsaOutput').value = encryptMessage(message, rsaKeys.publicKey);
});
...
document.getElementById('rsaDecryptBtn').addEventListener('click', () => {
    ...
    const cipher = document.getElementById('rsaOutput').value;
    if (!cipher) {
        alert('Enter cipher text to decrypt!');
        return;
    }
    document.getElementById('rsaOutput').value = decryptMessage(cipher, rsaKeys.privateKey);
});
